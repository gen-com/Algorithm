...
func solution(_ number: Int) {
    ...
    var queue = [(number: number, steps: 0, route: "\(number)")]
    var currentIndex = 0
    visited[number] = 0
    while currentIndex < queue.endIndex {
        ...
        if front.number % 3 == 0, front.steps + 1 < visited[front.number / 3] {
            ...
            queue.append((front.number / 3, front.steps + 1, front.route + " \(front.number / 3)"))
        }
        if front.number % 2 == 0, front.steps + 1 < visited[front.number / 2] {
            visited[front.number / 2] = front.steps + 1
            queue.append((front.number / 2, front.steps + 1, front.route + " \(front.number / 2)"))
        }
        if front.steps + 1 < visited[front.number - 1] {
            visited[front.number - 1] = front.steps + 1
            queue.append((front.number - 1, front.steps + 1, front.route + " \(front.number - 1)"))
        }
        currentIndex += 1
    }
}
